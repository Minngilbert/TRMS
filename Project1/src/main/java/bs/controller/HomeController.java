...
//		String s = request.getParameter(null);
		...
		if(request.getSession(false) == null || request.getSession().getAttribute("CurrUser") == null) {
			...
			if(emp.isDH()) {
				System.out.println("Is the DH "+emp.isDH());
				System.out.println("Reports to supervisor emp "+emp.getReportsTo()+" " + emp.getFirstName());
				RequestDispatcher rd = request.getRequestDispatcher("/HomeDH.html");
				rd.forward(request, response);
				
			}else if(emp.isBenCo()) {
				System.out.println("Is the BenCo "+emp.isBenCo());
				RequestDispatcher rd = request.getRequestDispatcher("/HomeBC.html");
				rd.forward(request, response);
			}else if(emp.isDS()) {
				System.out.println("Is the DS "+emp.isDS());
				...
				System.out.println("Is not the DH "+emp.isDH());
				System.out.println("Reports to normal emp "+emp.getReportsTo()+" " + emp.getFirstName());
...
//		String s = request.getParameter(null);
	...
	public static void submitTrmsForm(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
