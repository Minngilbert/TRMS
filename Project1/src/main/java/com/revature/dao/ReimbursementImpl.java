...
public class ReimbursementImpl implements ReimbursementDAO{
	...
	public ArrayList<Reimbursement> getAllReimbursements() {
		...
		try {
			...
			while(rs.next()){
				...
				temp.setEducationtype(rs.getString(" "));
    ...
    public void provideInformation(int applicationid,String info) {
        Connection cnn = cu.getConnection();
        
        try{
        	String sql ="UPDATE reimbursement SET moreinformation = ? WHERE applicationid = ?";
            PreparedStatement ps = cnn.prepareStatement(sql);
            ps.setString(1, info);
    ...
    public void requestInformation(int applicationid) {
        Connection cnn = cu.getConnection();
        try{
        	String sql = "UPDATE reimbursement SET moreinformation = true WHERE applicationid = ?";
            PreparedStatement ps = cnn.prepareStatement(sql);
            ps.setInt(1, applicationid);
            ps.executeUpdate();
    ...
    public boolean getEmpNotifications(int employeeid) throws SQLException {
			Connection cnn = cu.getConnection();
			...
			Reimbursement temp = new Reimbursement();
			if (rs.next())	{
				if(rs.getBoolean("dhrequestsinfo") || rs.getBoolean("bencorequestsinfo")) {
					return true;
				}
			}else {
